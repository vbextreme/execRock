...
inline void con_gotorc(uint_t r, uint_t c)
{
    printf("\033[%u;%uf",r,c);
}
...
void con_rect(uint_t r, uint_t c, uint_t h, uint_t w, uint_t style)
{
    ...
    utf8_put(al[0]);
    for (ic = 0; ic < w - 2 ; ++ic )
        utf8_put(boxStyle[style][UTF8_BOX_HORIZONTAL]);
    utf8_put(al[1]);
    ...
    {
        con_gotorc(ir,c);
        utf8_put(boxStyle[style][UTF8_BOX_VERTICAL]);
        con_gotorc(ir,c + w - 1);
        utf8_put(boxStyle[style][UTF8_BOX_VERTICAL]);
    }
}
